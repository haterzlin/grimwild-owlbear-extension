...
Status: **analysis complete; proposed plan for review; implementation not started.**
...
- Character data: [createEmptyCharacter](src/domain/characters.js) contains four stats/marks, Bloodied/Rattled, `story1/2`, `spark1/2`, XP, backgrounds/wises, arcs, traits/desires, bonds, complete selected talent objects, and biography. There is no character rules-version marker, Desperate field, or Weapon Style field.
- Persistence: [metadata.js](src/core/metadata.js) stores records under the existing character/pool/chat/GM namespaces. [AppShell](src/app/AppShell.js) normalizes characters on load and again before a debounced save. A harmless edit can therefore persist talent changes the player never selected.
- Current [path normalization](src/domain/paths.js), especially `normalizeTrackersFromTemplate`, matches tracker names and then falls back to position/first unused slot **without requiring matching type**. It drops unmatched tracker data, replaces stored talent properties with templates, and replaces cores by path even if the stored core is different. This older-character normalization is to be removed, not expanded into a CE migration layer.
...
- Phase 1 retirement targets: all six tests in `tests/compatibility.spec.js`; the single “normalizes legacy paladin tracker labels from saved 1.2 data” test and fixture import in `tests/paths.spec.js`; `tests/fixtures/legacy-1-2-characters.js` (imported only by those two suites); `package.json`'s `test:compatibility` script and its README command. Retain the other four path tests, `tests/helpers/characters.js`, debug helpers, and the mock runtime. No tests/fixtures are removed in Phase 0 and no legacy fixtures are added.
...
- [ ] Implement the CE marker and supported-record check from section 5 at the existing load/save boundary; show a short recreate-character notice when unsupported records are present.
- [ ] Remove legacy normalization, spelling aliases, and template-rewriting callers. Retain current talent initialization and direct CE persistence.
- [ ] Remove the legacy-only compatibility suite/fixtures and legacy Paladin test in the paths suite; remove unused package test commands or imports. Keep current-behavior coverage and add the minimal CE persistence/version-boundary checks.
